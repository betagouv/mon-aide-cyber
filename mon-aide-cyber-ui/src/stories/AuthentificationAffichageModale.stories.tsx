...
export const ModaleDeConnexion: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('Affiche la modale lorsque l’aidant clique sur "Se connecter"', async () => {
      await userEvent.click(canvas.getByRole('link', { name: /Se connecter/i }));

      expect(await waitFor(() => canvas.getByRole('textbox', { name: /votre adresse email/i }))).toBeInTheDocument();

      expect(await canvas.findByLabelText(/votre mot de passe/i)).toBeInTheDocument();

      expect(await waitFor(() => canvas.getByRole('button', { name: /annuler/i }))).toBeInTheDocument();

      expect(await waitFor(() => canvas.getByRole('button', { name: /se connecter/i }))).toBeInTheDocument();
    });

    await step("La modale disparaît lorsque l'aidant clique en dehors de la modale", async () => {
      await userEvent.click(document.body);

      await waitFor(() => expect(canvas.queryByText(/connectez vous/i)).not.toBeInTheDocument());
    });

    await step('La modale disparaît lorsque l’aidant clique sur le bouton "Annuler"', async () => {
      await userEvent.click(canvas.getByRole('link', { name: /Se connecter/i }));
    ...
    await step('Affiche "Veuillez saisir votre identifiant." lorsque l’aidant saisi un caractère vide', async () => {
      await userEvent.click(canvas.getByRole('link', { name: /Se connecter/i }));
      ...
      await waitFor(() =>
        expect(canvas.queryByText(/veuillez saisir votre identifiant de connexion./i)).toBeInTheDocument(),
      );
    });

    await step('Affiche "Veuillez saisir votre mot de passe." lorsque l’aidant saisi un caractère vide', async () => {
      await userEvent.click(canvas.getByRole('link', { name: /Se connecter/i }));
      ...
      await waitFor(() => expect(canvas.queryByText(/veuillez saisir votre mot de passe./i)).toBeInTheDocument());
    });
  },
};
