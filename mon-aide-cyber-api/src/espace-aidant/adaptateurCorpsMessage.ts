...
function genereCorpsConfirmationProfilAidantSansAssociation(aidant: Aidant) {
  return (
    ...
    `Si vous êtes membre d’une association non politique ou cultuelle, faites nous parvenir à l’adresse <a href="mailto:${adaptateurEnvironnement.messagerie().brevo().emailMAC()}">MonAideCyber</a> les informations suivantes :` +
    '\n' +
    '<ul>' +
    '<li>le nom</li>' +
    ...
    '</ul>' +
    '\n' +
    `Sinon contactez l’équipe à l’adresse mail <a href="mailto:${adaptateurEnvironnement.messagerie().brevo().emailMAC()}">MonAideCyber</a> si vous souhaitez  recevoir une liste d’associations déjà identifiées par l’équipe.` +
    '\n' +
    '\n' +
    'Toute l’équipe reste à votre disposition,' +
    '\n' +
    '\n' +
    `Pour toute remarque ou question, n’hésitez pas à nous contacter sur <a href="mailto:${adaptateurEnvironnement.messagerie().brevo().emailMAC()}">MonAideCyber</a> \n` +
    '</body>' +
    '</html>'
  );
}
